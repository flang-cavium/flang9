...
public:
  ...
  /// Set the maximum vector witdh for a Target.
  void setMaxVectorWidthForTarget(unsigned Val);

  /// True if TargetLibraryInfo uses a vectorized math library.
  void setHasExternalVectorMathLib(bool Val) {
    HasExternalVectorMathLib = Val;
  }

  /// Return the maximum vector witdh for a Target.
  unsigned getMaxVectorWidthForTarget() const;

  // Does TargetLibraryInfo use an external vectorized math library.
  bool hasExternalVectorMathLib() const {
    return HasExternalVectorMathLib;
  }
};
