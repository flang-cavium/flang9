...
static cl::opt<int>
AArch64RelaxPCRelBranch14("aarch64-relax-pcrel-branch14", cl::Hidden,
                          cl::init(32768),
...
static uint64_t adjustFixupValue(const MCFixup &Fixup, const MCValue &Target,
  ...
  case AArch64::fixup_aarch64_pcrel_branch14: {
    ...
    if (SignedValue > PV || SignedValue < NV) {
      std::string E("AArch64::fixup_aarch64_pcrel_branch14: ");
      E += "fixup value out of range: SV=";
      E += std::to_string(PV);
      E += " NV=";
      E += std::to_string(NV);
      Ctx.reportError(Fixup.getLoc(), E.c_str());
