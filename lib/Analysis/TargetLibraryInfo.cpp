...
static cl::opt<unsigned> MaxVectorWidthForTarget(
  "tli-max-vector-width-for-target",
  cl::Hidden, cl::init(0),
  cl::desc("Maximum vector width (in bits) for a specific Target."));

static cl::opt<bool> UseFastMath(
  "tli-use-fast-math",
  cl::Hidden, cl::init(false),
  cl::desc("Use fast-math vector library calls (if available)."));

static cl::opt<bool> UseFiniteMathOnly(
  "tli-use-finite-math-only",
  cl::Hidden, cl::init(false),
  cl::desc("Use finite-math-only vector library calls (if available)."));
...
TargetLibraryInfoImpl::TargetLibraryInfoImpl() {
  ...
  HasExternalVectorMathLib = false;
...
TargetLibraryInfoImpl::TargetLibraryInfoImpl(const TargetLibraryInfoImpl &TLI)
  ...
  HasExternalVectorMathLib(TLI.HasExternalVectorMathLib) {
...
void TargetLibraryInfoImpl::setFastMath(bool Val) {
  UseFastMath.setValue(Val);
}

void TargetLibraryInfoImpl::setFiniteMathOnly(bool Val) {
  UseFiniteMathOnly.setValue(Val);
}

void TargetLibraryInfoImpl::setMaxVectorWidthForTarget(unsigned Val) {
  MaxVectorWidthForTarget.setValue(Val);
}

unsigned TargetLibraryInfoImpl::getMaxVectorWidthForTarget() const {
  if (MaxVectorWidthForTarget.getValue())
    return MaxVectorWidthForTarget.getValue();
...
#define TLI_DEFINE_SLEEF_AARCH64_VECFUNCS_128
#include "llvm/Analysis/VecFuncs.def"
    };

    const VecDesc AArch64TwoAndFourLaneVecFuncsFastMath[] = {
#define TLI_DEFINE_SLEEF_AARCH64_VECFUNCS_FASTMATH_128
#include "llvm/Analysis/VecFuncs.def"
    };

    const VecDesc AArch64TwoAndFourLaneVecFuncsFiniteMath[] = {
#define TLI_DEFINE_SLEEF_AARCH64_VECFUNCS_FINITEMATH_128
#include "llvm/Analysis/VecFuncs.def"
    };

    if ((VF == 128) && ((TT.getArch() == Triple::aarch64) ||
                        (TT.getArch() == Triple::aarch64_be))) {
      if (UseFastMath)
        addVectorizableFunctions(AArch64TwoAndFourLaneVecFuncsFastMath);
      else if (UseFiniteMathOnly && !UseFastMath)
        addVectorizableFunctions(AArch64TwoAndFourLaneVecFuncsFiniteMath);
      else
  ...
  case PGMATH: {
    unsigned VF = getMaxVectorWidthForTarget();
    const VecDesc AArch64TwoAndFourLaneFortranVecFuncs[] = {
#define TLI_DEFINE_PGMATH_AARCH64_VECFUNCS_128
#include "llvm/Analysis/VecFuncs.def"
    };

    if ((VF == 128) && ((TT.getArch() == Triple::aarch64) ||
                        (TT.getArch() == Triple::aarch64_be)))
      addVectorizableFunctions(AArch64TwoAndFourLaneFortranVecFuncs);
    break;
  }
  case NoLibrary:
    break;
