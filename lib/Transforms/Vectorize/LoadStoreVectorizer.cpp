...
static cl::opt<unsigned> StackAdjustedAlignment(
  "load-store-vectorizer-stack-adjusted-alignment",
  cl::init(4), cl::Hidden,
  cl::desc("Stack Adjusted Alignment for Load-Store Vectorizer"));
static cl::opt<bool> AllowImplicitFloatVectorization(
  "load-store-vectorizer-allow-implicit-float",
  cl::init(true), cl::Hidden,
  cl::desc("Allow implicit float conversion in vectorization"));
...
bool LoadStoreVectorizerLegacyPass::runOnFunction(Function &F) {
  ...
  if (skipFunction(F) || F.hasFnAttribute(Attribute::NoImplicitFloat) ||
      !AllowImplicitFloatVectorization)
    return false;
