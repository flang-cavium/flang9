...
public:
  FlangMacroBuilder(ArgStringList &UpperCmdArgs, const ArgList &DriverArgs,
...
void FlangFrontend::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  if (Arg *A = Args.getLastArg(options::OPT_fsanitize_EQ)) {
    for (const StringRef &val : A->getValues()) {
      if (val.equals("discontiguous") || val.equals("undefined") ) {
        ...
        // -fsanitze=discontiguous has no meaning in LLVM, only flang
        // driver needs to recognize it. However -fsanitize=undefined
        // needs to be passed on for further processing by the non-flang
        // part of the driver.
  ...
  */
  ...
  Builder.defineMacro("__PTRDIFF_TYPE__",
                      TargetInfo::getTypeName(TI->getPtrDiffType(0)));
