...
static int fmtgbl_size = GBL_SIZE;
...
allocate_new_gbl()
{
  ...
  if (gbl_avl >= gbl_size) {
    if (gbl_size == GBL_SIZE) {
      ...
      (void) memcpy(tmp_gbl, gbl_head, sizeof(fioerror) * gbl_avl);
      gbl_head = tmp_gbl;
    } else {
      gbl_size += gbl_size == 0 ? GBL_SIZE : gbl_size;
      gbl_head = (fioerror *) realloc(gbl_head, sizeof(fioerror) * gbl_size);
    }
  }

  gbl = &gbl_head[gbl_avl];
  (void) memset(gbl, 0, sizeof(fioerror));
  ++gbl_avl;
}
...
allocate_new_fmtgbl()
{
  ...
  if (fmtgbl_avl >= fmtgbl_size) {
    if (fmtgbl_size == GBL_SIZE) {
      fmtgbl_size += fmtgbl_size;
      tmp_gbl = (f90fmt *) malloc(sizeof(f90fmt) * fmtgbl_size);
      (void) memcpy(tmp_gbl, fmtgbl_head, sizeof(f90fmt) * fmtgbl_avl);
      fmtgbl_head = tmp_gbl;
    } else {
      fmtgbl_size += fmtgbl_size == 0 ? GBL_SIZE : fmtgbl_size;
      fmtgbl_head =
  ...
  (void) memset(fmtgbl, 0, sizeof(f90fmt));
  ++fmtgbl_avl;
}
