...
ENTFTN(DATEA, datea)(DCHAR(date), F90_Desc *dated DCLEN64(date))
{
  ...
  (void) sprintf(loc_buf, "%2d-%3s-%02d", lt->tm_mday, month[lt->tm_mon],
                 yr2(lt->tm_year));
...
ENTFTN(FTIMEA, ftimea)(DCHAR(tbuf), F90_Desc *tbufd DCLEN64(tbuf))
{
  ...
  (void) sprintf(loc_buf, "%2.2d:%2.2d:%2.2d", ltimvar->tm_hour,
                 ltimvar->tm_min, ltimvar->tm_sec);
                     ...
                     F90_Desc *valuesd DCLEN64(date) DCLEN64(tbuf) DCLEN64(zone))
{
  ...
  if (tm == NULL) {
    (void) fprintf(__io_stderr(), "BAD return value from localtime(0x%lx)\n",
            (long)ltime);
    perror("localtime: ");
    exit(1);
  }

  (void) memcpy(&tmx, tm, sizeof(struct tm));
  tm = &tmx;
...
(DCHAR(res), DCHAR(expr) DCLEN64(res) DCLEN64(expr))
{
  ...
  */
...
#if defined(TARGET_LINUX_ARM) || defined(TARGET_LINUX_ARM64)
  i = CLEN(expr) - 1;
  if (i < 0)
    return 0;
  ...
  __builtin_strncpy(CADR(res), CADR(expr), (size_t) i + 1);
#else
  for (j = 0; j <= i; ++j)
    CADR(res)[j] = CADR(expr)[j];
#endif
  ...
  return i + 1;
#else // !(TARGET_LINUX_ARM | TARGET_LINUX_ARM64)
  char *rcptr;
  char *ecptr;
