; RUN: opt -mcpu=thunderx2t99 -vector-library=SLEEF -tli-max-vector-width-for-target=128 -loop-vectorize -S < %s | FileCheck %s
...
define void @atan2_f64(double* nocapture %varray) {
  ...
  ; CHECK:    [[TMP5:%.*]] = call <2 x double> @_ZGVnN2vv_atan2(<2 x double> [[TMP4:%.*]])
  ; CHECK:    ret void
  ;
  entry:
  br label %for.body
...
define void @atan2_f32(float* nocapture %varray) {
  ...
  ; CHECK:    [[TMP5:%.*]] = call <4 x float> @_ZGVnN4vv_atan2f(<4 x float> [[TMP4:%.*]])
  ; CHECK:    ret void
  ;
  entry:
  br label %for.body
...
define void @pow_f64(double* nocapture %varray) {
  ; CHECK-LABEL: @pow_f64(
  ; CHECK:    [[TMP5:%.*]] = call <2 x double> @_ZGVnN2vv_pow(<2 x double> [[TMP4:%.*]])
  ; CHECK:    ret void
  ;
  entry:
  br label %for.body
  ...
  %tmp = trunc i64 %iv to i32
  %conv = sitofp i32 %tmp to double
  %call = tail call double @pow(double %conv)
  %arrayidx = getelementptr inbounds double, double* %varray, i64 %iv
  store double %call, double* %arrayidx, align 8
  %iv.next = add nuw nsw i64 %iv, 1
  %exitcond = icmp eq i64 %iv.next, 1000
...
define void @pow_f32(float* nocapture %varray) {
  ; CHECK-LABEL: @pow_f32(
  ; CHECK:    [[TMP5:%.*]] = call <4 x float> @_ZGVnN4vv_powf(<4 x float> [[TMP4:%.*]])
  ; CHECK:    ret void
  ;
  entry:
  br label %for.body
  ...
  %tmp = trunc i64 %iv to i32
  %conv = sitofp i32 %tmp to float
  %call = tail call float @powf(float %conv)
  %arrayidx = getelementptr inbounds float, float* %varray, i64 %iv
  store float %call, float* %arrayidx, align 4
  %iv.next = add nuw nsw i64 %iv, 1
  %exitcond = icmp eq i64 %iv.next, 1000
